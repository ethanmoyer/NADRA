...
% algorithm (either "SVM_", "RF", or "CNN_"). It creates four arrays  
...
for i = 1:size(data)
	if (data.output{i} == "True" & data.test{i} == "True")
		true_true(a, :) = data(i, :);
		a = a + 1;
	elseif (data.output{i} == "True" & data.test{i} == "False")
		true_false(b, :) = data(i, :);
		b = b + 1;
	elseif (data.output{i} == "False" & data.test{i} == "True")
		false_true(c, :) = data(i, :);
		c = c + 1;
	elseif (data.output{i} == "False" & data.test{i} == "False")
